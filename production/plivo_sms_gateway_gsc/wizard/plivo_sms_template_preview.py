...
class PlivoSMSTemplatePreview(models.TransientModel):
    _name = "plivo.sms.template.preview"
    _description = "Plivo SMS Template Preview"
    ...
    def default_get(self, fields):
        result = super(PlivoSMSTemplatePreview, self).default_get(fields)
        sms_template_id = self.env.context.get('default_sms_template_id')
        if not sms_template_id or 'resource_ref' not in fields:
            return result
        sms_template = self.env['plivo.sms.template'].browse(sms_template_id)
        res = self.env[sms_template.model_id.model].search([], limit=1)
        if res:
            result['resource_ref'] = '%s,%s' % (sms_template.model_id.model, res.id)
        return result

    sms_template_id = fields.Many2one('plivo.sms.template', required=True, domain="[('model_id', '=', False)]", ondelete='cascade')
